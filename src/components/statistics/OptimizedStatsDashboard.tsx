...
export const OptimizedStatsDashboard: React.FC<OptimizedStatsDashboardProps> = ({
  ...
  const { visibleSections, trackElement } = useVisibilityTracker();
  ...
  const createSectionRef = useCallback((sectionId: string) => 
    (element: HTMLElement | null) => trackElement(element, sectionId),
    [trackElement]
  );
        ...
        >
          {onBack && (
            <Button
              ...
              variant="ghost"
              size="sm"
              className="glass-button h-10 w-10 md:h-12 md:w-12 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 hover:bg-white/20 transition-all duration-300 hover:scale-105"
            >
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-xs md:text-sm lg:text-base font-medium">
                {isMobile 
                  ? `${players.length} joueur${players.length > 1 ? 's' : ''} • ${roundCount} manche${roundCount > 1 ? 's' : ''}`
