...
export const StatsOverview: React.FC<StatsOverviewProps> = ({
  ...
  const calculateGlobalStats = () => {
    ...
    const averageScore = allScores.length ? Math.round(allScores.reduce((a, b) => a + b, 0) / allScores.length * 10) / 10 : 0;
    ...
    // Top performer
    const topPlayer = players.reduce((best, current) => current.totalScore < best.totalScore ? current : best);

    // Most consistent player
    const playerConsistency = players.map(player => {
      if (player.rounds.length < 2) return {
        player,
        consistency: 0
      };
    ...
    const mostConsistent = playerConsistency.reduce((best, current) => current.consistency < best.consistency ? current : best).player;
    return {
      averageScore,
      bestScore,
      worstScore,
  ...
  const keyMetrics = [{
    ...
    metrics: [{
      ...
      value: roundCount,
      suffix: roundCount === 1 ? "manche" : "manches",
      ...
      label: "Écart de score",
      value: stats.worstScore - stats.bestScore,
      suffix: "pts",
    ...
    metrics: [{
      label: "Champion actuel",
      value: stats.topPlayer.name,
      suffix: `(${stats.topPlayer.totalScore} pts)`,
      icon: <Trophy className="h-4 w-4" />
    }, {
      label: "Plus régulier",
      value: stats.mostConsistent.name,
      suffix: "",
      icon: <Target className="h-4 w-4" />
    }, {
      label: "Prises de risque",
      value: stats.highRiskRounds,
      suffix: "manches",
      icon: <Flame className="h-4 w-4" />
    }, {
      label: "Participation",
              ...
              return <motion.div key={player.id} initial={{
                ...
                delay: 0.5 + index * 0.1
                    ...
                    <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-muted/30 to-muted/10 backdrop-blur-sm rounded-xl border border-border/20 hover:shadow-lg transition-all duration-300 hover:scale-[1.01]">
                      {/* Position badge */}
                      <div className={`relative w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-lg ${globalRank === 1 ? 'bg-gradient-to-r from-yellow-500 to-orange-500' : globalRank === 2 ? 'bg-gradient-to-r from-gray-400 to-gray-600' : globalRank === 3 ? 'bg-gradient-to-r from-amber-600 to-yellow-700' : 'bg-gradient-to-r from-primary to-secondary'}`}>
