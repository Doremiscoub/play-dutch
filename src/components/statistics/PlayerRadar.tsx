...
export const PlayerRadar: React.FC<PlayerRadarProps> = ({
  ...
  const playersData = filteredPlayers.map((player, index) => {
    ...
    return {
      player,
      metrics,
      color: ['hsl(var(--primary))', 'hsl(var(--secondary))', 'hsl(var(--accent))', '#F97316', '#8B5CF6', '#10B981'][index % 6],
  ...
  const config = playersData.reduce((acc, {
    player,
    color
  }) => {
    acc[player.name] = {
  ...
  return <div className="space-y-6">
            ...
            <div className="h-96">
              <ChartContainer config={config}>
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart data={combinedData} margin={{
                  top: 20,
                  right: 80,
                  bottom: 20,
                  left: 80
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {playersData.map((playerData, index) => <motion.div key={playerData.player.id} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
