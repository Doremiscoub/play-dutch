...
export const useFunPlayerCard = ({
  ...
  // Memoïser les calculs coûteux avec une optimisation plus fine
  const cardData = useMemo(() => {
    const isWinner = rank === 1;
    const isLastPlace = rank === totalPlayers;
    const recentRounds = player.rounds.slice(-5);
    
    // Calculs de tendance optimisés
    ...
    // Calcul de la série actuelle avec optimisation
    const currentStreak = (() => {
      ...
      for (let i = player.rounds.length - 2; i >= 0; i--) {
        if (Math.abs(player.rounds[i].score - lastScore) <= 5) {
          streak++;
        } else {
          break;
        }
